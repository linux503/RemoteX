    ...
    match msg {
        ...
        ClientMsg::Accept {
            ...
            let session = inner
                ...
                .clone_ids();
            let caller = inner.devices.get(&session.0).map(|d| d.info.clone());
            let callee = inner.devices.get(&session.1).map(|d| d.info.clone());
            if let (Some(caller), Some(callee)) = (caller, callee) {
                if let Some(conn) = inner.devices.get(&session.0) {
                    let _ = conn.tx.send(ServerMsg::Accepted {
                        session_id: session_id.clone(),
                        peer: callee.clone(),
                    });
                }
                if let Some(conn) = inner.devices.get(&session.1) {
                    let _ = conn.tx.send(ServerMsg::Accepted {
                        session_id,
                        peer: caller,
                    });
...
impl Session {
    fn clone_ids(&self) -> (String, String) {
